# src/ingestion/json_collector.py (исправленный)
import json
    ...
    def collect_validation_report(self, results: List[ValidationResult], summary: dict) -> str:
        ...
        with open(out, "w", encoding="utf-8") as f:
            json.dump({
                ...
                "summary": summary,
                "details": [r.model_dump(mode="json") for r in results]  # ✅ ИСПРАВЛЕНО
            }, f, ensure_ascii=False, indent=2)
        return str(out)
    
    def collect_by_icd10(self, parsed: List[ParsedProtocol]) -> Dict[str, str]:
        out_dir = self.output_dir / "by_icd10"
        out_dir.mkdir(exist_ok=True)
        paths = {}
        for p in parsed:
            fp = out_dir / f"{p.metadata.icd10_code}.json"
            with open(fp, "w", encoding="utf-8") as f:
                json.dump(p.model_dump(mode="json"), f, ensure_ascii=False, indent=2)  # ✅
