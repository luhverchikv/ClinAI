...
class JSONCollector:
    ...
    def collect_protocols(self, parsed: List[ParsedProtocol]) -> str:
        ...
        with open(out, "w", encoding="utf-8") as f: json.dump({"exported_at": datetime.now().isoformat(), "count": len(parsed), "protocols": [p.model_dump() for p in parsed]}, f, ensure_ascii=False, indent=2)
        return str(out)
    ...
    def collect_by_icd10(self, parsed: List[ParsedProtocol]) -> Dict[str, str]:
        ...
        for p in parsed:
            ...
            with open(fp, "w", encoding="utf-8") as f: json.dump(p.model_dump(), f, ensure_ascii=False, indent=2)
